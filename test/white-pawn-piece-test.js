...
describe('White Pawn Piece Class', () => {
  describe('Default inherited properties', () => {
    ...
    it('White Pawn Piece has a xCoord', () => {
      expectOnPieceTest(MY_WHITE_PAWN_PIECE).to.have.property('xCoord');
      expectOnPieceTest(MY_WHITE_PAWN_PIECE.xCoord).to.be.a('number');
    });

    it('White Pawn Piece has a yCoord', () => {
      expectOnPieceTest(MY_WHITE_PAWN_PIECE).to.have.property('yCoord');
      expectOnPieceTest(MY_WHITE_PAWN_PIECE.yCoord).to.be.a('number');
    });
    ...
    it('Default xCoord is 0', () => {
      expectOnPieceTest(MY_WHITE_PAWN_PIECE.xCoord).to.be.equal(0);
    });

    it('Default yCoord is 0', () => {
      expectOnPieceTest(MY_WHITE_PAWN_PIECE.yCoord).to.be.equal(0);
    });
  ...
  describe('Non default property values', () => {
    ...
    it('Modifies default xCoord correctly', () => {
      expectOnPieceTest(MY_WHITE_PAWN_PIECE.xCoord).to.be.equal(5);
    });

    it('Modifies default yCoord correctly', () => {
      expectOnPieceTest(MY_WHITE_PAWN_PIECE.yCoord).to.be.equal(10);
    });
